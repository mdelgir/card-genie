...
test("Crazy Eights room sync exposes only owner hand and public discard metadata", async () => {
  ...
  try {
    ...
    for (let index = 0; index < clients.length; index++) {
      ...
      assert.equal(initial.G.hands["0"].length, index === 0 ? 5 : 0);
      assert.equal(initial.G.hands["1"].length, index === 1 ? 5 : 0);
    ...
    await waitFor(() => clients.every(client => client.getState()!.G.handCounts[actor] === 4));

    for (let index = 0; index < clients.length; index++) {
      const state = clients[index].getState()!;
      assert.deepEqual(state.G.deck, []);
      assert.deepEqual(state.G.discard, []);
      assert.equal(state.G.handCounts[actor], 4);
      assert.ok(state.G.discardTop);
      assert.equal(state.G.hands[actor].length, index === Number(actor) ? 4 : 0);
