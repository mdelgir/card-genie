...
export function GameBoard({ G, ctx, moves, matchData, playerID, matchID, credentials, isConnected }: BoardProps<SimpleCardGameState>) {
  ...
  const isTurn = playerID === ctx.currentPlayer && G.roundStatus === "playing";
  const name = (id: string) => matchData?.find(player => String(player.id) === id)?.name ?? `Player ${Number(id) + 1}`;
  const status = complete ? G.winner === "tie" ? "An even match. It's a tie." : G.winner !== null ? `${name(G.winner)} wins the round` : "Round complete" :
  ...
  return <section className={`board game-board${isTable ? " table" : ""}`}>
    <header className="board-heading">
      ...
      <span className={`round-badge${complete ? " round-badge--complete" : ""}`}>{complete ? "ROUND COMPLETE" : "HIGH CARD"}</span>
    </header>
    ...
    <div className={`felt-surface${isTable ? " felt-surface--public" : ""}`}>
      ...
      {!isTable && <div className="game-actions">
        {!complete ? <button type="button" onClick={() => moves.drawCard()} disabled={!isConnected || !isTurn || Boolean(ownCard)}>Draw card <span aria-hidden="true">↗</span></button> :
          canReplay ? <button type="button" onClick={() => moves.restartGame()} disabled={!isConnected}>Play again <span aria-hidden="true">↻</span></button> :
            <p role="status">Waiting for {name(ctx.currentPlayer)} to start the next round.</p>}
      </div>}
      ...
      <section className="players" aria-label="Public player cards">
        ...
        <ul className="player-card-grid">{Object.entries(G.hands).map(([id, card]) => {
          ...
          const current = G.roundStatus === "playing" && ctx.currentPlayer === id;
